...
#ifdef RT_LFS_DHARA_ENABLED
#include "stdbool.h"
#include "map.h"
#include "mem_section.h"
#ifdef RT_LFS_CUSTOM_INCLUDE_FILE
    #include RT_LFS_CUSTOM_INCLUDE_FILE
#endif /* RT_LFS_CUSTOM_INCLUDE_FILE */
...
static dhara_dev_t dhara_devs[RT_DEF_LFS_DRIVERS];
const static uint8_t dhara_devs_gc_ratio[RT_DEF_LFS_DRIVERS] =
{
    DHARA_DEV0_GC_RATIO,
#if RT_DEF_LFS_DRIVERS > 1
    DHARA_DEV1_GC_RATIO,
#endif
#if RT_DEF_LFS_DRIVERS > 2
    DHARA_DEV2_GC_RATIO,
#endif
#if RT_DEF_LFS_DRIVERS > 3
    DHARA_DEV3_GC_RATIO,
#endif
#if RT_DEF_LFS_DRIVERS > 4
    DHARA_DEV4_GC_RATIO,
#endif
#if RT_DEF_LFS_DRIVERS > 5
    DHARA_DEV5_GC_RATIO,
#endif
#if RT_DEF_LFS_DRIVERS > 6
    DHARA_DEV6_GC_RATIO,
#endif
#if RT_DEF_LFS_DRIVERS > 7
    DHARA_DEV7_GC_RATIO,
#endif
#if RT_DEF_LFS_DRIVERS > 8
    DHARA_DEV8_GC_RATIO,
#endif
#if RT_DEF_LFS_DRIVERS > 9
    DHARA_DEV9_GC_RATIO,
#endif
#if RT_DEF_LFS_DRIVERS > 10
#error "Not Supported"
#endif
...
static int _lfs_flash_read(const struct lfs_config *c, lfs_block_t block, lfs_off_t off, void *buffer, lfs_size_t size)
{
    ...
    dhara_dev = (dhara_dev_t *)c->context;
    // rt_kprintf("lfs_read:%d,%d\n", block ,size);
    while (size >= c->read_size)
    {
        if (dhara_map_read(&dhara_dev->map, block, buffer, &err))
        {
            rt_kprintf("dhara read failed: error: %d\n", err);
            return LFS_ERR_IO;
        }

        size -= c->read_size;
        buffer = (uint8_t *)buffer + c->read_size;
    }
    if (size > 0)
    {
        return LFS_ERR_IO;
...
static int _lfs_flash_prog(const struct lfs_config *c, lfs_block_t block, lfs_off_t off, const void *buffer, lfs_size_t size)
{
    ...
    // rt_kprintf("lfs_prog:%d,%d\n", block, size);
    dhara_dev = (dhara_dev_t *)c->context;
    while (size >= c->read_size)
    {
        if (dhara_map_write(&dhara_dev->map, block, buffer, &err))
        {
            rt_kprintf("dhara write failed: error: %d\n", err);
            return LFS_ERR_IO;
        }
...
static int _lfs_flash_sync(const struct lfs_config *c)
{
    ...
    ret = dhara_map_sync(&dhara_dev->map, &err);
    if (ret)
    {
...
static void _lfs_load_config(struct lfs_config *lfs_cfg, struct rt_device *mtd_dev)
{
    struct rt_mtd_nand_device *mtd_nand = (struct rt_mtd_nand_device *)mtd_dev;
    dhara_dev_t *dhara_dev = RT_NULL;
    dhara_error_t err = DHARA_E_NONE;
    int ret;
    uint32_t i;

    RT_ASSERT(2048 == mtd_nand->page_size);
    if (!lfs_cfg->context)
    {
        /* find an unused device */
        for (i = 0; i < RT_DEF_LFS_DRIVERS; i++)
        {
            if (!dhara_devs[i].page_buffer)
            {
                dhara_dev = &dhara_devs[i];
                break;
            }
        }
        RT_ASSERT(dhara_dev);
        lfs_cfg->context = dhara_dev;

        dhara_dev->page_buffer = rt_malloc(mtd_nand->page_size);
        RT_ASSERT(dhara_dev->page_buffer);

        dhara_dev->nand.num_blocks = mtd_nand->block_total;
        dhara_dev->nand.log2_page_size = 11; /* page_size = 2048 */
        dhara_dev->nand.log2_ppb = 6;        /* pages_per_block = 64, i.e. block_size = 128KB */
        dhara_dev->nand.user_data = (void *)mtd_dev;
        dhara_dev->nand.meta_buf = rt_malloc(mtd_nand->page_size);
        RT_ASSERT(dhara_dev->nand.meta_buf);

        dhara_map_init(&dhara_dev->map, &dhara_dev->nand, dhara_dev->page_buffer, dhara_devs_gc_ratio[i]);

        ret = dhara_map_resume(&dhara_dev->map, &err);
        rt_kprintf("map_resume:%d,%d\n", ret, err);
    }
    else
    {
        dhara_dev = (dhara_dev_t *)lfs_cfg->context;
    }

    lfs_cfg->read_size = mtd_nand->page_size;
    lfs_cfg->prog_size = mtd_nand->page_size;
    lfs_cfg->block_size = mtd_nand->page_size;
    lfs_cfg->cache_size = mtd_nand->page_size;
    lfs_cfg->block_cycles = LFS_BLOCK_CYCLES;
    lfs_cfg->block_count = dhara_map_capacity(&dhara_dev->map);
    rt_kprintf("lfs blk count:%d\n", lfs_cfg->block_count);
...
static void _lfs_load_deconfig(struct lfs_config *lfs_cfg)
{
    dhara_dev_t *dhara_dev = RT_NULL;
...
static int _get_disk(rt_device_t dev_id)
{
    ...
    else
    {
        for (index = 0; index < RT_DEF_LFS_DRIVERS; index ++)
        {
            if ((_lfs_mount_tbl[index] != RT_NULL) && (_lfs_mount_tbl[index]->cfg.context))
            {
                dhara_dev = (dhara_dev_t *)_lfs_mount_tbl[index]->cfg.context;
                if (dhara_dev->nand.user_data == (void *)dev_id)
                {
                    return index;
                }
            }
...
{
    uint64_t mtd_size;
    struct rt_mtd_nor_device *mtd_nor = (struct rt_mtd_nor_device *)mtd_dev;

    lfs_cfg->context = (void *)mtd_nor;

    if (mtd_nor->block_size < (128 * 1024))
    {
    ...
    {
        lfs_cfg->read_size = mtd_nor->sector_size;
        lfs_cfg->prog_size = mtd_nor->sector_size;
    }
    ...
    {
        lfs_cfg->cache_size = (LFS_CACHE_SIZE / lfs_cfg->read_size + 1) * lfs_cfg->read_size;
    }
    lfs_cfg->block_cycles = LFS_BLOCK_CYCLES;
...
static int _dfs_lfs_mount(struct dfs_filesystem *dfs, unsigned long rwflag, const void *data)
{
    ...
    _lfs_load_config(&dfs_lfs->cfg, dfs->dev_id);
    ...
    if (result != LFS_ERR_OK)
    {
        ...
        _lfs_load_deconfig(&dfs_lfs->cfg);
        /* release memory */
        rt_free(dfs_lfs);
...
static int _dfs_lfs_mkfs(rt_device_t dev_id)
{
    ...
    if (index == -1)
    {
        ...
        rt_memset(dfs_lfs, 0, sizeof(dfs_lfs_t));
        rt_mutex_init(&dfs_lfs->lock, "lfslock", RT_IPC_FLAG_PRIO);
        _lfs_load_config(&dfs_lfs->cfg, dev_id);
...
static int _dfs_lfs_statfs(struct dfs_filesystem *dfs, struct statfs *buf)
{
    ...
    /* Get used sectors */
    in_use = lfs_fs_size(&dfs_lfs->lfs);
