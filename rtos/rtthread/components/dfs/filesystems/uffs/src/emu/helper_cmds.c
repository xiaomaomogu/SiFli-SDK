...
int maketree(void)
{
	...
	int br;
	int bw;
	static HANDLE src_file;
	int dst_file;
	static BYTE buff[2048];

	dlen = strlen(dst_path);
	slen = strlen(src_path);
	sprintf(&src_path[slen], "/*");
	hdir = FindFirstFile(src_path, &find_data);		/* Open directory */
	if (hdir == INVALID_HANDLE_VALUE) {
		printf("Failed to open the source directory.\n");
	}
	else {
		for (;;) {
			sprintf(&src_path[slen], "/%s", find_data.cFileName);
			sprintf(&dst_path[dlen], "/%s", find_data.cFileName);
			if (find_data.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {	/* The item is a directory */
				if (strcmp(find_data.cFileName, ".") && strcmp(find_data.cFileName, "..")) {
					if (uffs_mkdir(dst_path) < 0) {	/* Create destination directory */
				...
				printf("%s\n", src_path);
				if ((src_file = CreateFile(src_path, GENERIC_READ, 0, 0, OPEN_EXISTING, 0, 0)) == INVALID_HANDLE_VALUE) {	/* Open source file */
					printf("Failed to open source file.\n"); break;
				}

				dst_file = uffs_open(dst_path, UO_CREATE | UO_RDWR);
				if (dst_file < 0) {	/* Create destination file */
					printf("Failed to create destination file: %d.\n", dst_file); break;
				}
				do {	/* Copy source file to destination file */
					ReadFile(src_file, buff, sizeof(buff), &br, 0);
					if (br == 0) break;
					bw = uffs_write(dst_file, buff, br);
				} while (br == bw);
...
static int cmd_import(int argc, char *argv[])
{
	...
	strncpy(src_path, argv[1], sizeof(src_path) - 1);
...
static const struct cli_command helper_cmds[] = 
{
	...
	{ cmd_import,	"import",		"<import_path>",    "import directory", },
    { NULL, NULL, NULL, NULL }
};
