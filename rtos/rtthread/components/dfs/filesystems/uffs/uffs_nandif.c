...
//  attr->total_blocks = nand->end_block - nand->start_block + 1;/* no use */
    ...
    attr->spare_size = nand->oob_size;                     /* page spare size */
    attr->ecc_opt = RT_CONFIG_UFFS_ECC_MODE;               /* ecc option */
    attr->ecc_size = nand->oob_size-nand->oob_free;        /* ecc size, not used actually by AUTO mode */
    attr->block_status_offs = 0;                           /* indicate block bad or good, offset in spare */
    attr->layout_opt = RT_CONFIG_UFFS_LAYOUT;              /* let UFFS do the spare layout */

    /* calculate the ecc layout array, it's used by uffs_FlashUnloadSpare and uffs_FlashMakeSpare */
    hw_flash_data_layout[0] = 2;    /* 2 bytes bad block marker */
    hw_flash_data_layout[1] = 0x06; /* first 6bytes of tagStore */
    hw_flash_data_layout[2] = 18;   /* skip 8bytes ecc and 2 bytes bad block marker */
    hw_flash_data_layout[3] = 0x02; /* remaining 2bytes of tagStore */   
    hw_flash_data_layout[4] = 0xFF;
    hw_flash_data_layout[5] = 0x00;

    hw_flash_ecc_layout[0] = 8;     /* ecc for one sector only */
    hw_flash_ecc_layout[1] = 8;     /* 6bytes user data ecc and 2byte spare data ecc */
