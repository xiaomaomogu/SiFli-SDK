...
int dfs_ramfs_ioctl(struct dfs_fd *file, int cmd, void *args)
{
    if (F_GET_PHY_ADDR == cmd)
    {
        if (args)
        {
            ...
            *((uint32_t *)args) = (uint32_t)(&(dirent->data[file->pos]));
            return 0;
        }
    }
    else if (F_RESERVE_CONT_SPACE == cmd)
    {
        ...
        struct dfs_ramfs *ramfs;
        rt_uint8_t *ptr;
        rt_uint32_t size = (rt_uint32_t)args;
        dirent = (struct ramfs_dirent *)file->data;
        RT_ASSERT(dirent != NULL);

        ramfs = dirent->fs;
        RT_ASSERT(ramfs != NULL);
        ptr = rt_memheap_realloc(&(ramfs->memheap), dirent->data, size);
        if (ptr == NULL)
        {
            rt_set_errno(-ENOMEM);
            return -ENOMEM;
...
int dfs_ramfs_flush(struct dfs_fd *file)
{
    return 0;
}
