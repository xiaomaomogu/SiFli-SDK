 ...
 * @macro uassert_true_ret      if @value is true,     not assert, means passing, return with void if @value is false.
...
#define uassert_true_ret(value)  do {__utest_assert(value, "(" #value ") is false"); \
                                     if(utest_handle_get()->failed_num != 0) return; } while(0)
