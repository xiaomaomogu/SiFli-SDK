 ...
 */
static void rt_sensor_cb(rt_sensor_t sen)
{
...
static rt_err_t rt_sensor_open(rt_device_t dev, rt_uint16_t oflag)
{
    ...
    if (oflag & RT_DEVICE_FLAG_RDONLY && dev->flag & RT_DEVICE_FLAG_RDONLY)
    {
        ...
        r = sensor->ops->control(sensor, RT_SENSOR_CTRL_SET_MODE, (void *)RT_SENSOR_MODE_POLLING) ;
        if (r== RT_EOK)
        {
    ...
    else if (oflag & RT_DEVICE_FLAG_INT_RX && dev->flag & RT_DEVICE_FLAG_INT_RX)
    {
        ...
        r= sensor->ops->control(sensor, RT_SENSOR_CTRL_SET_MODE, (void *)RT_SENSOR_MODE_INT);
        if ( r == RT_EOK)
        {
    ...
    {
        r= sensor->ops->control(sensor, RT_SENSOR_CTRL_SET_MODE, (void *)RT_SENSOR_MODE_FIFO);
    ...
    if (r==RT_EOK) {
        ...
        r=sensor->ops->control(sensor, RT_SENSOR_CTRL_SET_POWER, (void *)RT_SENSOR_POWER_NORMAL);
        if ( r== RT_EOK)
        {
 ...
 * sensor register
 */
__ROM_USED int rt_hw_sensor_register(rt_sensor_t sensor,
                                     const char              *name,
                                     rt_uint32_t              flag,
                                     void                    *data)
{
