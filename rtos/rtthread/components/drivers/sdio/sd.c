...
static rt_int32_t mmcsd_parse_scr(struct rt_mmcsd_card *card)
{
    ...
    //LOG_I("scr : 0x%08x, 0x%08x", card->resp_scr[0], card->resp_scr[1]);
    LOG_I("SD card version %d, bus width %d.", scr->sd_version, scr->sd_bus_widths);
...
rt_int32_t mmcsd_get_scr(struct rt_mmcsd_card *card, rt_uint32_t *scr)
{
    ...
    rt_kprintf("Before scr buf 0x%08x: scr 0x%08x, 0x%08x\n", (uint32_t)scr, scr[0], scr[1]);
    scr[0] = be32_to_cpu(scr[0]);
    scr[1] = be32_to_cpu(scr[1]);
    rt_kprintf("After scr 0x%08x, 0x%08x\n", scr[0], scr[1]);
                                     ...
                                     rt_uint32_t           ocr)
{
    ...
    rt_kprintf("hs flag 0x%x, card max %d\n", card->flags, card->hs_max_data_rate);
    ...
    /*switch bus width*/
    LOG_I("4 bit mode: host 0x%x, card 0x%x\n", host->flags, card->scr.sd_bus_widths);
