...
#define DATA_FLOW_CONTROL           0x0052
#define DATA_FLOW_END               0x00FE
...
struct udcd_ops
{
    ...
    rt_err_t (*test_mode)(rt_uint16_t tm, rt_uint8_t *data, rt_uint8_t len);
};
...
rt_inline rt_err_t dcd_test_mode(udcd_t dcd, rt_uint16_t test_mode, rt_uint8_t *data, rt_uint8_t len)
{
    RT_ASSERT(dcd != RT_NULL);
    RT_ASSERT(dcd->ops->test_mode != RT_NULL);
    RT_ASSERT((test_mode & 0xff) == 0);

    return dcd->ops->test_mode(test_mode, data, len);
}
