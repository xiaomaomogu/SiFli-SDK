...
static rt_size_t _inquiry_cmd(ufunction_t func, ustorage_cbw_t cbw)
{
    ...
    *(rt_uint32_t*)&buf[4] = 31;
    ...
    //rt_print_data(buf, 0, SIZEOF_INQUIRY_CMD);
    RT_DEBUG_LOG(RT_DEBUG_USB, ("cb_data_size: %d\n", data->cb_data_size));
...
static rt_err_t _function_enable(ufunction_t func)
{
    ...
    if((res = rt_device_open(data->disk, RT_DEVICE_OFLAG_RDWR)) != RT_EOK)
    {
        // for flash, device has been opened for fils system, ignore busy status
        if(!(res == (-RT_EBUSY) ))
        {
