...
static rt_err_t _ep_out_handler(ufunction_t func, rt_size_t size)
{
    ...
    rt_device_t dev= (rt_device_t)winusb_device;
    rt_size_t rx_size;
    
    rx_size=rt_ringbuffer_put(winusb_device->rx_fifo, winusb_device->rx_buf, size);
    if(winusb_device->parent.rx_indicate != RT_NULL)
    {
        winusb_device->parent.rx_indicate(&winusb_device->parent, rx_size);
    }

    if (rx_size<size) {
        rt_kprintf("RX FIFO Full %d<%d\n", rx_size,size);
    }
    winusb_device->ep_out->request.buffer=winusb_device->rx_buf;
...
static rt_size_t win_usb_read(rt_device_t dev, rt_off_t pos, void *buffer, rt_size_t size)
{
    ...
    {
        rt_kprintf("Device not configured in read\n");
        return 0;
    }
    winusb_device_t winusb_device = (winusb_device_t)dev;

    int rd_size=rt_ringbuffer_get(winusb_device->rx_fifo, buffer, size);
...
static rt_size_t win_usb_write(rt_device_t dev, rt_off_t pos, const void *buffer, rt_size_t size)
{
    if(((ufunction_t)dev->user_data)->device->state != USB_STATE_CONFIGURED)
    {
        rt_kprintf("Device not configured in write\n");
        return 0;
    }
    winusb_device_t winusb_device = (winusb_device_t)dev;
    ...
    size=rt_usbd_io_request(((ufunction_t)dev->user_data)->device,winusb_device->ep_in,&winusb_device->ep_in->request);
    return size;
}
...
rt_err_t win_usb_open(rt_device_t dev, rt_uint16_t oflag)
{
    winusb_device_t winusb_device = (winusb_device_t)dev;

    RT_ASSERT(winusb_device->rx_fifo==RT_NULL);
    winusb_device->rx_fifo=rt_ringbuffer_create(WINUSB_FIFO_SIZE);

    winusb_device->ep_out->buffer = winusb_device->rx_buf;
    winusb_device->ep_out->request.buffer = winusb_device->rx_buf;
    winusb_device->ep_out->request.size = winusb_device->ep_out->ep_desc->wMaxPacketSize;
...
rt_err_t win_usb_close(rt_device_t dev)
{
    winusb_device_t winusb_device = (winusb_device_t)dev;
    if (winusb_device->rx_fifo) {
        rt_ringbuffer_destroy(winusb_device->rx_fifo);
        winusb_device->rx_fifo=RT_NULL;
    }
    return RT_EOK;
}
...
static rt_err_t rt_usb_winusb_init(ufunction_t func)
{
    ...
    winusb_device->parent.open      = win_usb_open;
    winusb_device->parent.close     = win_usb_close;
    ...
    rt_memset(winusb_device->rx_buf,0,sizeof(winusb_device->rx_buf));
