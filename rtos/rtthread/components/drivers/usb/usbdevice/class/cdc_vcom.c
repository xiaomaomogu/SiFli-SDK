...
static rt_err_t _interface_handler(ufunction_t func, ureq_t setup)
{
    ...
    RT_DEBUG_LOG(RT_DEBUG_USB, ("vcom _interface_handler :%d \n", setup->bRequest));
    ...
    case CDC_SET_CONTROL_LINE_STATE:
        data->connected = RT_TRUE;//(setup->wValue & 0x01) > 0 ? RT_TRUE : RT_FALSE;
...
ufunction_t rt_usbd_function_cdc_create(udevice_t device)
{
    ...
    vcom_data = (struct vcom *)rt_malloc(sizeof(struct vcom));
    RT_ASSERT(vcom_data != RT_NULL);
    rt_memset(vcom_data, 0, sizeof(struct vcom));
    func->user_data = (void *)vcom_data;
    ...
    vcom_data->ep_cmd = rt_usbd_endpoint_new(&comm_desc->ep_desc, _ep_cmd_handler);

    /* add the command endpoint to the cdc communication interface */
    rt_usbd_altsetting_add_endpoint(comm_setting, vcom_data->ep_cmd);
    ...
    vcom_data->ep_out = rt_usbd_endpoint_new(&data_desc->ep_out_desc, _ep_out_handler);
    vcom_data->ep_in = rt_usbd_endpoint_new(&data_desc->ep_in_desc, _ep_in_handler);

    /* add the bulk out and bulk in endpoints to the data alternate setting */
    rt_usbd_altsetting_add_endpoint(data_setting, vcom_data->ep_in);
    rt_usbd_altsetting_add_endpoint(data_setting, vcom_data->ep_out);
...
uint8_t rt_usbd_vcom_getepnum(uint8_t ep)
{
    RT_ASSERT(vcom_data != RT_NULL);
    if (ep) //in
    {
        rt_kprintf("%s %d in_id_addr=%d\n", __func__, __LINE__, vcom_data->ep_in->id->addr);
        return vcom_data->ep_in->id->addr;
    }
    else //out
    {
        rt_kprintf("%s %d in_id_addr=%d\n", __func__, __LINE__, vcom_data->ep_out->id->addr);
        return vcom_data->ep_out->id->addr;
    }
}

//data_dir-> Data direction  1 or 0 ;1->in 0->out
//flag-> 1:continue receive; 0:stop
void _vcom_flow_control(uint8_t data_dir, uint8_t flag)
{
    uint8_t ep_num = rt_usbd_vcom_getepnum(data_dir);
    HAL_PCD_Set_RxbuffControl(ep_num, flag);
    if (flag) HAL_PCD_Set_RxscrACK(ep_num);
}
...
{
    uint8_t res = 0;
    struct flow_data *data_ep = (struct flow_data *)arg;
    ...
    case DATA_FLOW_CONTROL:
        _vcom_flow_control(data_ep->dir, data_ep->flag);
...
static void vcom_tx_thread_entry(void *parameter)
{
    ...
    while (1)
    {
        ...
        rt_device_t cdc_dev = (rt_device_t)func->user_data;
        if (cdc_dev->tx_complete != RT_NULL)
        {
            cdc_dev->tx_complete(cdc_dev, (void *)DATA_FLOW_END);
        }
    }
}
...
static void rt_usb_vcom_init(struct ufunction *func)
{
    ...
    config.hwfc         = RT_SERIAL_HWFC_NONE;
