...
__ROM_USED rt_err_t  rt_pipe_open(rt_device_t device, rt_uint16_t oflag)
{
...
__ROM_USED rt_err_t  rt_pipe_close(rt_device_t device)
{
    rt_pipe_t *pipe = (rt_pipe_t *)device;
...
__ROM_USED rt_size_t rt_pipe_read(rt_device_t device, rt_off_t pos, void *buffer, rt_size_t count)
{
    uint8_t *pbuf;
...
__ROM_USED rt_size_t rt_pipe_write(rt_device_t device, rt_off_t pos, const void *buffer, rt_size_t count)
{
    uint8_t *pbuf;
...
__ROM_USED rt_err_t  rt_pipe_control(rt_device_t dev, int cmd, void *args)
{
    return RT_EOK;
}
...
__ROM_USED rt_pipe_t *rt_pipe_create(const char *name, int bufsz)
{
    rt_pipe_t *pipe;
    rt_device_t dev;
...
__ROM_USED int rt_pipe_delete(const char *name)
{
    int result = 0;
    rt_device_t device;
...
#ifdef RT_USING_POSIX
__ROM_USED int pipe(int fildes[2])
{
    rt_pipe_t *pipe;
    char dname[8];
    char dev_name[32];
...
__ROM_USED int mkfifo(const char *path, mode_t mode)
{
    rt_pipe_t *pipe;
