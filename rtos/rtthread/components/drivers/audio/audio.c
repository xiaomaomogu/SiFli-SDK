...
static rt_err_t _audio_dev_open(struct rt_device *dev, rt_uint16_t oflag)
{
    ...
    if (oflag & RT_DEVICE_OFLAG_RDONLY)
    {
        if (audio->record == RT_NULL)
        {
            ...
            //create pipe for record
            {
                rt_size_t pipe_size = CFG_AUDIO_RECORD_PIPE_SIZE;

                RT_ASSERT((rt_strlen(audio->dev_name) + rt_strlen("_recpipe")) < 32);
                rt_memcpy(pipe_name, audio->dev_name, rt_strlen(audio->dev_name));
                rt_memcpy(&pipe_name[rt_strlen(audio->dev_name)], "_recpipe", rt_strlen("_recpipe"));

                if (rt_strstr(audio->dev_name, "pdm") || rt_strstr(audio->dev_name, "i2s"))
                {
                    pipe_size = CFG_AUDIO_RECORD_PIPE_SIZE * 2;
                }
            ...
            audio->record = record;
            audio->record->audio_pipe = (struct rt_audio_pipe *)rt_device_find(pipe_name);
            if (audio->record->audio_pipe == RT_NULL)
            {
                AUDIO_DBG("find audio pipe error, %s\n", pipe_name);
...
static rt_err_t _audio_dev_close(struct rt_device *dev)
{
    ...
    if (dev->open_flag & RT_DEVICE_OFLAG_RDONLY)
    {
        ...
        if (audio->record != RT_NULL)
        {
            rt_device_close(RT_DEVICE(audio->record->audio_pipe));
            rt_audio_pipe_destroy(audio->record->audio_pipe);
            rt_free(audio->record);
            audio->record = RT_NULL;
        }
...
static rt_size_t _audio_dev_read(struct rt_device *dev, rt_off_t pos, void *buffer, rt_size_t size)
{
    ...
    return rt_device_read(RT_DEVICE(audio->record->audio_pipe), pos, buffer, size);
}
...
__ROM_USED rt_err_t rt_audio_register(struct rt_audio_device *audio, const char *name, rt_uint32_t flag, void *data)
{
    struct rt_device *device;
    RT_ASSERT(audio != RT_NULL);
    device = &(audio->parent);

    audio->dev_name = (char *)name;
...
__ROM_USED int rt_audio_samplerate_to_speed(rt_uint32_t bitValue)
{
    int speed = 0;
    switch (bitValue)
    {
...
__ROM_USED rt_uint32_t rt_audio_format_to_bits(rt_uint32_t format)
{
    switch (format)
    {
...
__ROM_USED void rt_audio_tx_complete(struct rt_audio_device *audio, rt_uint8_t *pbuf)
{
    rt_err_t result;
...
__ROM_USED void rt_audio_rx_done(struct rt_audio_device *audio, rt_uint8_t *pbuf, rt_size_t len)
{
    //save data to record pipe
    rt_device_write(RT_DEVICE(RT_DEVICE(audio->record->audio_pipe)), 0, pbuf, len);
