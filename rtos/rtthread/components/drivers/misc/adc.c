...
static rt_err_t _adc_control(rt_device_t dev, int cmd, void *args)
{
    ...
    {
        result = rt_adc_enable(adc, (rt_uint32_t)args);
    }
    else if (cmd == RT_ADC_CMD_DISABLE)
    {
        result = rt_adc_disable(adc, (rt_uint32_t)args);
    }
#ifdef RT_USING_PM
    else if (RT_DEVICE_CTRL_RESUME == cmd)
    {
        ...
        if (PM_SLEEP_MODE_STANDBY == power_mode)
        {
            ...
            if (adc->channel != ADC_INVALID_CHANNEL)
            {
                adc->ops->enabled(adc, adc->channel, RT_TRUE);
            }
        }
    }
...
rt_err_t rt_hw_adc_register(rt_adc_device_t device, const char *name, const struct rt_adc_ops *ops, const void *user_data)
{
    ...
    device->channel = ADC_INVALID_CHANNEL;
...
rt_err_t rt_adc_disable(rt_adc_device_t dev, rt_uint32_t channel)
{
    ...
    if (RT_EOK == result)
    {
        dev->channel = ADC_INVALID_CHANNEL;
    }
