...
static rt_err_t i2c_bus_device_open(rt_device_t dev, rt_uint16_t oflag)
{
    struct rt_i2c_bus_device *bus = (struct rt_i2c_bus_device *)dev->user_data;

    RT_ASSERT(bus != RT_NULL);

    return rt_i2c_open(bus, oflag);
}
...
__ROM_USED rt_err_t rt_i2c_bus_device_device_init(struct rt_i2c_bus_device *bus,
        const char               *name,
        rt_uint32_t              flag)
{
