...
#ifdef __arm__
int dlmodule_relocate(struct rt_dlmodule *module, Elf32_Rel *rel, Elf32_Addr sym_val, uint8_t module_mode)
{
    ...
    {
        rt_uint32_t temp_value;
        temp_value = *(Elf32_Addr *)((rt_uint8_t *)module->exec_mem_space
                                     + rel->r_offset
                                     - module->vstart_addr);
        where = &temp_value;
    }
