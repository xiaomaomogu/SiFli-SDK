...
/* stack */
__ROM_USED union finsh_value   finsh_vm_stack[FINSH_STACK_MAX];
/* text segment */
__ROM_USED uint8_t             text_segment[FINSH_TEXT_MAX];

__ROM_USED union finsh_value  *finsh_sp;       /* stack pointer */
__ROM_USED uint8_t            *finsh_pc;       /* PC */

/* syscall list, for dynamic system call register */
__ROM_USED struct finsh_syscall_item *global_syscall_list = NULL;

// #define FINSH_VM_DISASSEMBLE
__ROM_USED void finsh_vm_run()
{
    uint8_t op;
...
#ifdef RT_USING_HEAP
__ROM_USED void finsh_syscall_append(const char *name, syscall_func func)
{
    /* create the syscall */
...
#ifdef FINSH_VM_DISASSEMBLE
__ROM_USED void finsh_disassemble()
{
    uint8_t *pc, op;
