...
/* finsh thread */
    ...
    L1_NON_RET_BSS_SECT_BEGIN(finsh_thread_stack)
    ALIGN(RT_ALIGN_SIZE)
    L1_NON_RET_BSS_SECT(finsh_thread_stack, static char finsh_thread_stack[FINSH_THREAD_STACK_SIZE]);
    L1_NON_RET_BSS_SECT_END
    struct finsh_shell _shell;
#endif

__ROM_USED struct finsh_shell *shell;
static char *finsh_prompt_custom = RT_NULL;

#ifdef RT_USING_HEAP
__ROM_USED int finsh_set_prompt(const char *prompt)
{
...
__ROM_USED const char *finsh_get_prompt()
{
 ...
 */
__ROM_USED rt_uint32_t finsh_get_prompt_mode(void)
{
 ...
 */
__ROM_USED void finsh_set_prompt_mode(rt_uint32_t prompt_mode)
{
    RT_ASSERT(shell != RT_NULL);
...
#if defined(RT_USING_POSIX) && !defined(CFG_FACTORY_DEBUG)
    return getchar();
#else
    char ch = 0;
 ...
 */
__ROM_USED void finsh_set_device(const char *device_name)
{
 ...
 */
__ROM_USED const char *finsh_get_device()
{
    RT_ASSERT(shell != RT_NULL);
 ...
 */
__ROM_USED void finsh_set_echo(rt_uint32_t echo)
{
    RT_ASSERT(shell != RT_NULL);
 ...
 */
__ROM_USED rt_uint32_t finsh_get_echo()
{
    RT_ASSERT(shell != RT_NULL);
 ...
 */
__ROM_USED rt_err_t finsh_set_password(const char *password)
{
    rt_ubase_t level;
 ...
 * @return password
 */
__ROM_USED const char *finsh_get_password(void)
{
...
#ifndef FINSH_USING_MSH_ONLY
__ROM_USED void finsh_run_line(struct finsh_parser *parser, const char *line)
{
    const char *err_str;
...
__ROM_USED void finsh_thread_entry(void *parameter)
{
    int ch;
...
__ROM_USED void finsh_system_function_init(const void *begin, const void *end)
{
    _syscall_table_begin = (struct finsh_syscall *) begin;
    _syscall_table_end = (struct finsh_syscall *) end;
}

__ROM_USED void finsh_system_var_init(const void *begin, const void *end)
{
 ...
 */
__ROM_USED int finsh_system_init(void)
{
    ...
    tid = rt_thread_create(FINSH_THREAD_NAME,
                           ...
                           FINSH_THREAD_STACK_SIZE, FINSH_THREAD_PRIORITY, RT_THREAD_TICK_DEFAULT);
#else
    shell = &_shell;
    tid = &finsh_thread;
