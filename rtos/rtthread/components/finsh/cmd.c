...
static long _list_thread(struct rt_list_node *list)
{
    ...
    rt_kprintf("%-*.s pri  status      sp      top     stack size max used left tick  error\n", maxlen, item_title);
    object_split(maxlen);
    rt_kprintf(" ---  ------- ---------- ---------- ----------  ------  ---------- ---\n");
    for (node = list->next; node != list; node = node->next)
    {
        ...
        rt_kprintf(" 0x%08x 0x%08x 0x%08x    %02d%%   0x%08x %03d\n",
                   (rt_uint32_t)thread->sp, //((rt_uint32_t)thread->sp - (rt_uint32_t)thread->stack_addr),
        ...
        rt_kprintf(" 0x%08x 0x%08x 0x%08x    %02d%%   0x%08x %03d\n",
                   (rt_uint32_t)thread->sp, //(thread->stack_size + (rt_uint32_t)thread->stack_addr - (rt_uint32_t)thread->sp),
                   (rt_uint32_t)thread->stack_addr + thread->stack_size - 4,
                   thread->stack_size,
...
__ROM_USED long list_thread(void)
{
...
__ROM_USED long list_event(void)
{
    struct rt_object_information *info;
...
__ROM_USED long list_mutex(void)
{
    struct rt_object_information *info;
...
__ROM_USED long list_mailbox(void)
{
    struct rt_object_information *info;
...
__ROM_USED long list_msgqueue(void)
{
    struct rt_object_information *info;
...
__ROM_USED long list_memheap(void)
{
    struct rt_object_information *info;
...
__ROM_USED long list_mempool(void)
{
    struct rt_object_information *info;
...
__ROM_USED long list_timer(void)
{
    struct rt_object_information *info;
...
__ROM_USED long list_device(void)
{
    struct rt_object_information *info;
...
__ROM_USED void list_prefix(char *prefix)
{
    struct finsh_syscall_item *syscall_item;
    struct finsh_sysvar_item *sysvar_item;
