...
ALIGN(RT_ALIGN_SIZE)
__ROM_USED uint8_t finsh_heap[FINSH_HEAP_MAX];
...
__ROM_USED int finsh_heap_init(void)
{
    /* clear heap to zero */
 ...
 */
__ROM_USED void *finsh_heap_allocate(size_t size)
{
 ...
 */
__ROM_USED void  finsh_heap_free(void *ptr)
{
    struct finsh_block_header *header;
