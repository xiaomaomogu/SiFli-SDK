...
__ROM_USED rt_sighandler_t rt_signal_install(int signo, rt_sighandler_t handler)
{
    rt_base_t level;
...
__ROM_USED void rt_signal_mask(int signo)
{
    rt_base_t level;
    rt_thread_t tid = rt_thread_self();
...
__ROM_USED void rt_signal_unmask(int signo)
{
    rt_base_t level;
    rt_thread_t tid = rt_thread_self();
...
__ROM_USED int rt_signal_wait(const rt_sigset_t *set, rt_siginfo_t *si, rt_int32_t timeout)
{
    int ret = RT_EOK;
    rt_base_t   level;
    rt_thread_t tid = rt_thread_self();
    ...
    RT_ASSERT(tid->error != -RT_ETIMEOUT);
...
__ROM_USED void rt_thread_handle_sig(rt_bool_t clean_state)
{
    rt_base_t level;
...
__ROM_USED void rt_thread_alloc_sig(rt_thread_t tid)
{
    int index;
    rt_base_t level;
...
__ROM_USED void rt_thread_free_sig(rt_thread_t tid)
{
    rt_base_t level;
    struct siginfo_node *si_node;
...
__ROM_USED int rt_thread_kill(rt_thread_t tid, int sig)
{
    siginfo_t si;
    rt_base_t level;
    struct siginfo_node *si_node;
...
__ROM_USED int rt_system_signal_init(void)
{
