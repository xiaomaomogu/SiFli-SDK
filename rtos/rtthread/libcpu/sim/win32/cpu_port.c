...
rt_thread_t rt_thread_self(void)
{
    ...
    struct rt_thread *thread, *r = NULL;
    DWORD tid = GetCurrentThreadId();
    win_thread_t  *pWinThread;
    ...
    for (node = list->next; node != list; node = node->next)
    {
        ...
        pWinThread = (win_thread_t *) thread->sp;
        if (pWinThread && tid == pWinThread->ThreadID)
        {
            r = thread;
            break;
        }
...
void WinThreadScheduler(void)
{
    ...
    while (1)
    {
        ...
        if (SwitchRequiredMask != 0)
        {
            ...
            if (WinThreadTo != NULL)
            {
                int count = ResumeThread(WinThreadTo->ThreadHandle);
                while (count > 0)
                {
                    count = ResumeThread(WinThreadTo->ThreadHandle);
                }
            }
        }
