...
static void thread_suspend_signal_handler(int sig)
{
    ...
    if (sig != MSG_SUSPEND)
    {
        rt_kprintf("get an unexpected signal <%d>, exit\n", sig);
        exit(EXIT_FAILURE);
    }
    ...
    thread_t *thread, void *task, void *parameter, void *pexit)
{
    ...
    {
        rt_kprintf("init thread->sem failed, exit \n");
        exit(EXIT_FAILURE);
    }
    ...
    if (res)
    {
        rt_kprintf("pthread create faild, <%d>\n", res);
        exit(EXIT_FAILURE);
    }
...
void rt_hw_interrupt_enable(rt_base_t level)
{
    ...
    if (thread_from->pthread == pid)
    {
        ...
        {
            rt_kprintf("conswitch: should not be here! %d\n", __LINE__);
            exit(EXIT_FAILURE);
        }
...
static int mainthread_scheduler(void)
{
    ...
    for (;;)
    {
        ...
        if (sigwait(&sigmask, &sig) != 0)
        {
            rt_kprintf("mthread: sigwait get unexpected sig %d\n", sig);
        }
#else
        pause();
#endif
