...
#endif /* COREMARK_MININUM_RUN_TIME */
...
#error "Please define a way to initialize a memory block."
...
#endif
	if (time_in_secs(total_time) < COREMARK_MININUM_RUN_TIME) {
	...
	if (total_errors==0) {
		...
		if (known_id==3) {
			secs_ret t;
			int score;
			t = time_in_secs(total_time);
			if (0 == t) {
				score = 0;
			}
			else {
				score = default_num_contexts*results[0].iterations/t;
			}
			ee_printf("CoreMark 1.0 : %d / %s %s",score,COMPILER_VERSION,COMPILER_FLAGS);
#if defined(MEM_LOCATION) && !defined(MEM_LOCATION_UNSPEC)
			ee_printf(" / %s",MEM_LOCATION);
#else
			ee_printf(" / %s",mem_name[MEM_METHOD]);
