...
bool lv_draw_mask_is_only_map_mask(const lv_area_t * a, lv_draw_mask_map_param_t * p)
{
    uint8_t i;
    lv_draw_mask_map_param_t *tmp_p = NULL;
    for(i = 0; i < _LV_MASK_MAX_NUM; i++) 
    {
        _lv_draw_mask_common_dsc_t * comm_param = LV_GC_ROOT(_lv_draw_mask_list[i]).param;
        if(comm_param == NULL) continue;
        if(LV_DRAW_MASK_TYPE_MAP == comm_param->type)
        {
            if(NULL == tmp_p)
            {
                tmp_p = (lv_draw_mask_map_param_t *) LV_GC_ROOT(_lv_draw_mask_list[i]).param;

                if(!_lv_area_is_on(a,&tmp_p->cfg.coords))  tmp_p = NULL;
            }
            else
            {
                //More than 1 map mask present
                return false;
            }
        }
        else if(LV_DRAW_MASK_TYPE_RADIUS == comm_param->type) 
        {
            lv_draw_mask_radius_param_t * radius_param = LV_GC_ROOT(_lv_draw_mask_list[i]).param;
            if(radius_param->cfg.outer) {
                if(!_lv_area_is_out(a, &radius_param->cfg.rect, radius_param->cfg.radius)) return false;
            }
            else {
                if(!_lv_area_is_in(a, &radius_param->cfg.rect, radius_param->cfg.radius)) return false;
            }

            ;//a is full visible, ignore this mask.
        }
        else 
        {
            //Not map mask
    ...
    if(tmp_p)
    {
        if(p)
        {
            memcpy(p, tmp_p, sizeof(lv_draw_mask_map_param_t));
        }
