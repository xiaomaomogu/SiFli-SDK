...
static inline lv_color_t color_blend_true_color_multiply(lv_color_t fg, lv_color_t bg, lv_opa_t opa);
    ...
    #define lv_color_mix_func              lv_color_mix_fill
    #define lv_color_premult_func          lv_color_premult_fill
    #define lv_color_mix_premult_func      lv_color_mix_premult_fill
    #define lv_color_mix_with_alpha_func   lv_color_mix_with_alpha_fill
#else
    #define lv_color_mix_func              lv_color_mix
    #define lv_color_premult_func          lv_color_premult
    #define lv_color_mix_premult_func      lv_color_mix_premult
    #define lv_color_mix_with_alpha_func   lv_color_mix_with_alpha
...
#define FILL_NORMAL_MASK_PX(color)                                                          \
    ...
    else *dest_buf = lv_color_mix_func(color, *dest_buf, *mask);            \
    mask++;                                                         \
    dest_buf++;
...
#define MAP_NORMAL_MASK_PX(x)                                                          \
    if(*mask_tmp_x) {          \
        ...
        else dest_buf[x] = lv_color_mix_func(src_buf[x], dest_buf[x], *mask_tmp_x);            \
    }                                                                                               \
    mask_tmp_x++;
                                              ...
                                              lv_coord_t dest_stride, lv_color_t color, lv_opa_t opa, const lv_opa_t * mask, lv_coord_t mask_stride)
{
    ...
    if(mask == NULL) {
        ...
        /*Has opacity*/
            ...
            lv_color_t last_res_color = lv_color_mix_func(color, last_dest_color, opa);

#if LV_COLOR_MIX_ROUND_OFS == 0 && LV_COLOR_DEPTH == 16
            /*lv_color_mix_func work with an optimized algorithm with 16 bit color depth.
             *However, it introduces some rounded error on opa.
             *Introduce the same error here too to make lv_color_premult_func produces the same result */
            opa = (uint32_t)((uint32_t)opa + 4) >> 3;
            opa = opa << 3;
#endif

            uint16_t color_premult[3];
            lv_color_premult_func(color, opa, color_premult);
#ifdef COMPATIBLE_WITH_SIFLI_EPIC_FILL
            lv_opa_t opa_inv = 256 - opa;
#else
            ...
            for(y = 0; y < h; y++) {
                for(x = 0; x < w; x++) {
                    if(!lv_color_eq(last_dest_color, dest_buf[x])) {
                        ...
                        last_res_color = lv_color_mix_premult_func(color_premult, dest_buf[x], opa_inv);
                    }
                ...
                for(x = 0; x < w; x++) {
                    if(*mask) {
                        ...
                        if(*mask != last_mask || !lv_color_eq(last_dest_color, dest_buf[x])) {
                            ...
                            else last_res_color = lv_color_mix_func(color, dest_buf[x], opa_tmp);
                            last_mask = *mask;
...
static inline void set_px_argb(uint8_t * buf, lv_color_t color, lv_opa_t opa)
{
...
#if LV_COLOR_DEPTH == 8
    bg_color.full = buf[0];
    lv_color_mix_with_alpha_func(bg_color, bg_opa, color, opa, &res_color, &buf[1]);
    if(buf[1] <= LV_OPA_MIN) return;
    buf[0] = res_color.full;
#elif LV_COLOR_DEPTH == 16
    bg_color.full = buf[0] + (buf[1] << 8);
    lv_color_mix_with_alpha_func(bg_color, bg_opa, color, opa, &res_color, &buf[2]);
    ...
    lv_color_mix_with_alpha_func(bg_color, bg_opa, color, opa, &res_color, &buf[3]);
...

{
    ...
    if(mask == NULL) {
        if(opa >= LV_OPA_MAX) {
            ...
            for(y = 0; y < h; y++) {
                for(x = 0; x < w; x++) {
                    dest_buf[x] = lv_color_mix_func(src_buf[x], dest_buf[x], opa);
                }
                dest_buf += dest_stride;
                src_buf += src_stride;
            }
            ...
            for(y = 0; y < h; y++) {
                for(x = 0; x < w; x++) {
                    if(mask[x]) {
                        ...
                        dest_buf[x] = lv_color_mix_func(src_buf[x], dest_buf[x], opa_tmp);
                    }
                }
                dest_buf += dest_stride;
                src_buf += src_stride;
...
static inline lv_color_t color_blend_true_color_additive(lv_color_t fg, lv_color_t bg, lv_opa_t opa)
{
    ...
    return lv_color_mix_func(fg, bg, opa);
}
