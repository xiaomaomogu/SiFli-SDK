...
void lv_canvas_set_buffer(lv_obj_t * obj, void * buf, lv_coord_t w, lv_coord_t h, lv_img_cf_t cf)
{
    ...
    canvas->dsc.data      = buf;
    canvas->dsc.data_size = lv_img_buf_get_img_size(w, h, cf);
...
void lv_canvas_fill_bg(lv_obj_t * canvas, lv_color_t color, lv_opa_t opa)
{
    ...
    if(dsc->header.cf == LV_IMG_CF_INDEXED_1BIT) {
        ...
        lv_memset((uint8_t *)dsc->data + 8, color.ch.blue ? 0xff : 0x00, row_byte_cnt * dsc->header.h);
#else    
...
#else
    lv_draw_ctx_t * draw_ctx = lv_mem_alloc(drv->draw_ctx_size);
    LV_ASSERT_MALLOC(draw_ctx);
    if(draw_ctx == NULL) return;
    drv->draw_ctx_init(drv, draw_ctx);
