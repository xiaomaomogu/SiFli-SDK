...
#define LV_COLOR_SET1(dst, src)   (dst).full = (src).full
#define LV_COLOR_SET8(dst, src)   (dst).full = (src).full
#define LV_COLOR_SET16(dst, src)  (dst).full = (src).full
#define LV_COLOR_SET24(dst, src)                                          \
                                  do                                      \
                                  {                                       \
...
#define LV_COLOR_SET32(dst, src)  (dst).full = (src).full
...
static inline uint32_t lv_color_to32(lv_color_t color)
{
    ...
    ret.ch.green = color.ch.green;
    ret.ch.red = color.ch.red;
    ret.ch.alpha = 0xFF;
    return ret.full;    
#elif LV_COLOR_DEPTH == 32
    return color.full;
...
LV_ATTRIBUTE_FAST_MEM static inline bool lv_color_eq(lv_color_t c1, lv_color_t c2)
{
#if LV_COLOR_DEPTH == 24
    if ((c1.ch.red == c2.ch.red) && (c1.ch.green == c2.ch.green) && (c1.ch.blue == c2.ch.blue)) return true;
    else return false;
#else
    return (c1.full == c2.full);
#endif
}
                                                                 ...
                                                                 lv_color_t * res_color, lv_opa_t * res_opa)
{
    ...
    /*Both colors have alpha. Expensive calculation need to be applied*/
        ...
        if(fg_opa != fg_opa_save || bg_opa != bg_opa_save || !lv_color_eq(fg_color, fg_color_save) ||
           !lv_color_eq(bg_color, bg_color_save)) {
            fg_opa_save        = fg_opa;
            bg_opa_save        = bg_opa;
...
#ifdef COMPATIBLE_WITH_SIFLI_EPIC_FILL
#define EPIC_EXPAND_TO_8BIT(x, v)  ((((uint8_t)(x)) << (v))|(((uint8_t)(x)) >> (8 - (v) - (v))))
#define GET_EPIC_R8(c) EPIC_EXPAND_TO_8BIT(LV_COLOR_GET_R(c), 3)
#define GET_EPIC_G8(c) EPIC_EXPAND_TO_8BIT(LV_COLOR_GET_G(c), 2)
#define GET_EPIC_B8(c) EPIC_EXPAND_TO_8BIT(LV_COLOR_GET_B(c), 3)

LV_ATTRIBUTE_FAST_MEM static inline lv_color_t lv_color_mix_fill(lv_color_t c1, lv_color_t c2, uint8_t mix)
{
 lv_color_t ret;
#if (16 == LV_COLOR_DEPTH)
 /*LV_COLOR_DEPTH == 16*/
 LV_COLOR_SET_R(ret, ((uint16_t) GET_EPIC_R8(c1) * mix + GET_EPIC_R8(c2) * (256 - mix)) >> 11);
 LV_COLOR_SET_G(ret, ((uint16_t) GET_EPIC_G8(c1) * mix + GET_EPIC_G8(c2) * (256 - mix)) >> 10);
 LV_COLOR_SET_B(ret, ((uint16_t) GET_EPIC_B8(c1) * mix + GET_EPIC_B8(c2) * (256 - mix)) >> 11);
 LV_COLOR_SET_A(ret, 0xFF);
#elif LV_COLOR_DEPTH != 1
 /*LV_COLOR_DEPTH == 8 or 32*/
 LV_COLOR_SET_R(ret, ((uint16_t) LV_COLOR_GET_R(c1) * mix + LV_COLOR_GET_R(c2) * (256 - mix)) >> 8);
 LV_COLOR_SET_G(ret, ((uint16_t) LV_COLOR_GET_G(c1) * mix + LV_COLOR_GET_G(c2) * (256 - mix)) >> 8);
 LV_COLOR_SET_B(ret, ((uint16_t) LV_COLOR_GET_B(c1) * mix + LV_COLOR_GET_B(c2) * (256 - mix)) >> 8);
 LV_COLOR_SET_A(ret, 0xFF);
#else
 /*LV_COLOR_DEPTH == 1*/
 ret.full = mix > LV_OPA_50 ? c1.full : c2.full;
...
LV_ATTRIBUTE_FAST_MEM static inline void lv_color_premult_fill(lv_color_t c, uint8_t mix, uint16_t *out)
{
#if (16 == LV_COLOR_DEPTH)
 out[0] = (uint16_t) GET_EPIC_R8(c) * mix;
 out[1] = (uint16_t) GET_EPIC_G8(c) * mix;
 out[2] = (uint16_t) GET_EPIC_B8(c) * mix;
#elif LV_COLOR_DEPTH != 1
 out[0] = (uint16_t) LV_COLOR_GET_R(c) * mix;
 out[1] = (uint16_t) LV_COLOR_GET_G(c) * mix;
 out[2] = (uint16_t) LV_COLOR_GET_B(c) * mix;
...
LV_ATTRIBUTE_FAST_MEM static inline lv_color_t lv_color_mix_premult_fill(uint16_t *premult_c1, lv_color_t c2, uint8_t mix)
{
 lv_color_t ret;
#if (16 == LV_COLOR_DEPTH)
 /*LV_COLOR_DEPTH == 16*/
 LV_COLOR_SET_R(ret, ((uint16_t) premult_c1[0] + GET_EPIC_R8(c2) * mix) >> 11);
 LV_COLOR_SET_G(ret, ((uint16_t) premult_c1[1] + GET_EPIC_G8(c2) * mix) >> 10);
 LV_COLOR_SET_B(ret, ((uint16_t) premult_c1[2] + GET_EPIC_B8(c2) * mix) >> 11);
 LV_COLOR_SET_A(ret, 0xFF);
#elif LV_COLOR_DEPTH != 1
 /*LV_COLOR_DEPTH == 8, 16 or 32*/
 LV_COLOR_SET_R(ret, ((uint16_t) premult_c1[0] + LV_COLOR_GET_R(c2) * mix) >> 8);
 LV_COLOR_SET_G(ret, ((uint16_t) premult_c1[1] + LV_COLOR_GET_G(c2) * mix) >> 8);
 LV_COLOR_SET_B(ret, ((uint16_t) premult_c1[2] + LV_COLOR_GET_B(c2) * mix) >> 8);
 LV_COLOR_SET_A(ret, 0xFF);
#else
 /*LV_COLOR_DEPTH == 1*/
 /*Restore color1*/
...
* @param res_opa the result opacity
*/
LV_ATTRIBUTE_FAST_MEM static inline void lv_color_mix_with_alpha_fill(lv_color_t bg_color, lv_opa_t bg_opa,
     lv_color_t fg_color, lv_opa_t fg_opa,
     lv_color_t *res_color, lv_opa_t *res_opa)
{
 /* Pick the foreground if it's fully opaque or the Background is fully transparent*/
 ...
 {
     *res_color = lv_color_mix_fill(fg_color, bg_color, fg_opa);
     *res_opa = LV_OPA_COVER;
 }
 /*Both colors have alpha. Expensive calculation need to be applied*/
 else
 {
             ...
             !lv_color_eq(bg_color, bg_color_save))
     {
         ...
         lv_opa_t ratio = (uint16_t)((uint16_t)fg_opa * 255) / res_opa_saved;
         res_color_saved  = lv_color_mix_fill(fg_color, bg_color, ratio);
...
#endif /* COMPATIBLE_WITH_SIFLI_EPIC_FILL */
