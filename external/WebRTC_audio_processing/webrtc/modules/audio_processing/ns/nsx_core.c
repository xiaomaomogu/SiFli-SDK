                             ...
                             int16_t *q_noise)
{
    ...
    int16_t *lmagn = malloc(HALF_ANAL_BLOCKL * 2);
    RT_ASSERT(lmagn);
#else
    int16_t lmagn[HALF_ANAL_BLOCKL];
#endif
...
#ifdef AUDIO_MEM_ALLOC
    int16_t *winData_buff = malloc((ANAL_BLOCKL_MAX * 2 + 16) * 2);
    int16_t *realImag_buff = malloc((ANAL_BLOCKL_MAX * 2 + 16) * 2);
    RT_ASSERT(winData_buff && realImag_buff);
#else
...
#ifdef AUDIO_MEM_ALLOC
    int16_t *realImag_buff = malloc((ANAL_BLOCKL_MAX * 2 + 16) * 2);
    int16_t *rfft_out_buff = malloc((ANAL_BLOCKL_MAX * 2 + 16) * 2);
    RT_ASSERT(realImag_buff && rfft_out_buff);
#else
    int16_t realImag_buff[(ANAL_BLOCKL_MAX * 2 + 16)];
                           ...
                           short *const *outFrame)
{
...
#ifdef AUDIO_MEM_ALLOC
    uint32_t *noiseU32 = malloc(HALF_ANAL_BLOCKL * 4);
    uint32_t *postLocSnr = malloc(HALF_ANAL_BLOCKL * 4);
    uint32_t *priorLocSnr = malloc(HALF_ANAL_BLOCKL * 4);
    uint32_t *prevNearSnr = malloc(HALF_ANAL_BLOCKL * 4);
    uint16_t *magnU16 = malloc(HALF_ANAL_BLOCKL * 2);
    uint16_t *prevNoiseU16 = malloc(HALF_ANAL_BLOCKL * 2);
    uint16_t *nonSpeechProbFinal = malloc(HALF_ANAL_BLOCKL * 2);
    uint16_t *noiseSupFilterTmp = malloc(HALF_ANAL_BLOCKL * 2);
    RT_ASSERT(noiseU32 && postLocSnr && priorLocSnr && prevNearSnr &&
              magnU16 && prevNoiseU16 && nonSpeechProbFinal && noiseSupFilterTmp);
#else
    uint16_t magnU16[HALF_ANAL_BLOCKL];
    uint16_t prevNoiseU16[HALF_ANAL_BLOCKL];
    uint16_t nonSpeechProbFinal[HALF_ANAL_BLOCKL * 2];
    ...
    if (inst->zeroInputSignal)
    {
        ...
        free(magnU16);
        free(prevNoiseU16);
        free(nonSpeechProbFinal);
        free(noiseSupFilterTmp);
#endif
        return;
    }
