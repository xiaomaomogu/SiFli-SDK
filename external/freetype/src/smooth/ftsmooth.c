...
#define SCALE  ( 1 << 2 )
                           ...
                           TOrigin*        target )
  {
    ...
    if (8 != ft_get_bpp())
    {
        ...
        rt_assert_handler("should not coming here", __FUNCTION__, __LINE__);
        return;
    }
...
#undef SCALE
                    ...
                    const FT_Vector*  origin )
  {
    ...
    if (2 == bpp || 4 == bpp)
    {
      //ref FT_ALLOC_MULT -> FT_MEM_ALLOC_MULT--> ft_mem_realloc
      if (FT_REALLOC( bitmap->buffer, 0, bitmap->rows * bitmap->pitch * bpp / 8 + 1))
        goto Exit;
    }
    else if ( FT_ALLOC_MULT( bitmap->buffer, bitmap->rows, bitmap->pitch ) )
      goto Exit;
    ...
    if ( mode == FT_RENDER_MODE_NORMAL ||
         mode == FT_RENDER_MODE_LIGHT  )
    {
      if ( (outline->flags & FT_OUTLINE_OVERLAP)  && (bpp == 8))
