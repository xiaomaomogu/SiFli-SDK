...
#if 0   // Found different result between BSP and reference calculation.
#define NN_ROUND(out_shift) ( (0x1u << out_shift) >> 1 )
#else   // Change 0x1u to (q31_t)1 or (uint16_t) 1 could fix the issue.
#define NN_ROUND(out_shift) (((q31_t)1 << out_shift) >> 1)
#endif
#else
