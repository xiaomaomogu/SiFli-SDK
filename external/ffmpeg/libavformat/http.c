...
#define sizeof_hoststr      1024
#define sizeof_path1        MAX_URL_SIZE
#define sizeof_buf          1024
#define sizeof_urlbuf       MAX_URL_SIZE
#define sizeof_auth         1024
#define sizeof_proxyauth    1024
#define sizeof_pathbuf      1024
...
static int http_open_cnx_internal(URLContext *h, AVDictionary **options)
{
...
#if VERY_BIG_STACK_SIZE
    char hostname[sizeof_hostname], hoststr[sizeof_hoststr], proto[10];
    char auth[sizeof_auth], proxyauth[sizeof_proxyauth] = "";
    char path1[sizeof_path1];
    char buf[sizeof_buf], urlbuf[sizeof_urlbuf];
#else
    char proto[10];
    char *hostname = av_mallocz(sizeof_hostname);
    char *hoststr = av_mallocz(sizeof_hoststr);
    char *auth = av_mallocz(sizeof_auth);
    char *proxyauth = av_mallocz(sizeof_proxyauth);
    char *path1 = av_mallocz(sizeof_path1);
    char *buf = av_mallocz(sizeof_buf);
    char *urlbuf = av_mallocz(sizeof_urlbuf);
    RT_ASSERT(hostname && hoststr && auth && proxyauth && path1 && buf && urlbuf);
#endif

    int port, use_proxy, err, location_changed = 0;
    HTTPContext *s = h->priv_data;

    av_url_split(proto, sizeof(proto), auth, sizeof_auth,
                 hostname, sizeof_hostname, &port,
                 path1, sizeof_path1, s->location);

    ff_url_join(hoststr, sizeof_hoststr, NULL, NULL, hostname, port, NULL);
    ...
    if (use_proxy) {
        ...
        ff_url_join(urlbuf, sizeof_urlbuf, proto, NULL, hostname, port, "%s",
                    path1);
        path = urlbuf;
        av_url_split(NULL, 0, proxyauth, sizeof_proxyauth,
                     hostname, sizeof_hostname, &port, NULL, 0, proxy_path);
    }

    ff_url_join(buf, sizeof_buf, lower_proto, NULL, hostname, port, NULL);
...
static int http_write_reply(URLContext* h, int status_code)
{
    ...
    message = av_malloc(MAX_URL_SIZE);
    RT_ASSERT(message);
    if (status_code < 0)
        body = 1;
    switch (status_code) {
    ...
    if (body) {
        s->chunked_post = 0;
        message_len = snprintf(message, MAX_URL_SIZE,
                 "HTTP/1.1 %03d %s\r\n"
                 "Content-Type: %s\r\n"
                 "Content-Length: %zu\r\n"
                 "\r\n"
...
static int http_listen(URLContext *h, const char *uri, int flags,
    ...
    int port;
    hostname = av_mallocz(sizeof_hostname);
    av_assert0(hostname);

    av_url_split(proto, sizeof(proto), NULL, 0, hostname, sizeof_hostname, &port,
                 NULL, 0, uri);
    if (!strcmp(proto, "https"))
        lower_proto = "tls";
...
{
    char *redirected_location, *new_loc;
    redirected_location = av_malloc(MAX_URL_SIZE);
    RT_ASSERT(redirected_location);
    ff_make_absolute_url(redirected_location, MAX_URL_SIZE,
                         s->location, p);
...
static int http_read_header(URLContext *h, int *new_location)
{
    ...
    char *line;
    int err = 0;
    line = av_malloc(MAX_URL_SIZE);
    RT_ASSERT(line);
    s->chunksize = UINT64_MAX;

    for (;;) {
        if ((err = http_get_line(s, line, MAX_URL_SIZE)) < 0) {
            av_free(line);
            return err;
        }
                        ...
                        const char *proxyauth, int *new_location)
{
    ...
    int ret;
    headers = av_mallocz(HTTP_HEADERS_SIZE);
    if (!headers)
    {
        rt_kprintf("% %d no mem", __FUNCTION__, __LINE__);
        return -1;
    }
    /* send http header */
    ...
    if (!has_header(s->headers, "\r\nUser-Agent: "))
        len += av_strlcatf(headers + len, HTTP_HEADERS_SIZE - len,
                           "User-Agent: %s\r\n", s->user_agent);
    if (!has_header(s->headers, "\r\nAccept: "))
        len += av_strlcpy(headers + len, "Accept: */*\r\n",
                          HTTP_HEADERS_SIZE - len);
    ...
    if (s->headers)
        av_strlcpy(headers + len, s->headers, HTTP_HEADERS_SIZE - len);
    ...
    if (strlen(headers) + 1 == HTTP_HEADERS_SIZE ||
        ret >= sizeof(s->buffer)) {
...
static int64_t http_seek_internal(URLContext *h, int64_t off, int whence, int force_reconnect)
{
    ...
    if (s->off && h->is_streamed)
        return AVERROR(ENOSYS);
    old_buf = av_mallocz(BUFFER_SIZE);
    RT_ASSERT(old_buf);
...
static int http_proxy_open(URLContext *h, const char *uri, int flags)
{
    HTTPContext *s = h->priv_data;
#if VERY_BIG_STACK_SIZE
    char hostname[sizeof_hostname], hoststr[sizeof_hoststr];
    char auth[sizeof_auth], pathbuf[sizeof_pathbuf],;
#else
    char *hostname, *hoststr, *auth, *pathbuf;
    hostname = av_mallocz(sizeof_hostname);
    hoststr = av_mallocz(sizeof_hoststr);
    auth = av_mallocz(sizeof_auth);
    pathbuf = av_mallocz(sizeof_pathbuf);
#endif
    char *path;
    char lower_url[100];
    ...
    av_url_split(NULL, 0, auth, sizeof_auth, hostname, sizeof_hostname, &port,
                 pathbuf, sizeof_pathbuf, uri);
    ff_url_join(hoststr, sizeof_hoststr, NULL, NULL, hostname, port, NULL);
    path = pathbuf;
    if (*path == '/')
        path++;
