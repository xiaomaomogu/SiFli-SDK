...
static void mov_metadata_creation_time(AVDictionary **metadata, int64_t time)
{
    ...
    if (time)
    {
        ...
        tz = tz_get();
        timet += tz->tz_minuteswest * 60L;
        ptm = gmtime_r(&timet, &tmbuf);
#else
        ptm = localtime(&timet);
#endif
        /*
        time_data_t cur_time;
        ...
        cur_time.ms = 0;
        cur_time.wday = get_day_of_week(cur_time.year, cur_time.month, cur_time.day);
        */
        if (!ptm) return;
...
static int mov_switch_root(AVFormatContext *s, int64_t target)
{
    ...
    ret = mov_read_default(mov, s->pb, (MOVAtom)
    {
