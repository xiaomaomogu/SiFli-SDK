    ...
    for (;;) {
...

        {
            ...
            if (o_named && o_named->type == AV_OPT_TYPE_CONST)
                ...
                if (o->unit) {
                    for (o_named = NULL; (o_named = av_opt_next(target_obj, o_named))!=NULL; ) {
...
static int set_string_pixel_fmt(void *obj, const AVOption *o, const char *val, uint8_t *dst)
{
    return set_string_fmt(obj, o, val, dst,
                          AV_PIX_FMT_NB, (int (*)(char const *))av_get_pix_fmt, "pixel format");
}

static int set_string_sample_fmt(void *obj, const AVOption *o, const char *val, uint8_t *dst)
{
    return set_string_fmt(obj, o, val, dst,
                          AV_SAMPLE_FMT_NB, (int (*)(char const *))av_get_sample_fmt, "sample format");
}
...
{
    return get_format(obj, name, search_flags, (int *)out_fmt, AV_OPT_TYPE_PIXEL_FMT, "pixel");
}

int av_opt_get_sample_fmt(void *obj, const char *name, int search_flags, enum AVSampleFormat *out_fmt)
{
    return get_format(obj, name, search_flags, (int *)out_fmt, AV_OPT_TYPE_SAMPLE_FMT, "sample");
}
