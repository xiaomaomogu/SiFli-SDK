...
#include "version.h"
    ...
    #undef MINI_PIXDESC_SIZE
#elif CONFIG_MJPEGB_DECODER
    #undef MINI_PIXDESC_SIZE
#else
    #define MINI_PIXDESC_SIZE 1
#endif
...
#endif
#if MINI_PIXDESC_SIZE
static const AVPixFmtDescriptor av_pix_fmt_descriptors_yuv420p = {
    .name = "yuv420p",
    .nb_components = 3,
    .log2_chroma_w = 1,
    .log2_chroma_h = 1,
...
static const AVPixFmtDescriptor av_pix_fmt_descriptors_rgb565le = {
    .name = "rgb565le",
    .nb_components = 3,
    .log2_chroma_w = 0,
    .log2_chroma_h = 0,
...
static const AVPixFmtDescriptor av_pix_fmt_descriptors_rgb24 = {
    .name = "rgb24",
    .nb_components = 3,
    .log2_chroma_w = 0,
    .log2_chroma_h = 0,
...
static const AVPixFmtDescriptor av_pix_fmt_descriptors_bgr24 = {
    .name = "bgr24",
    .nb_components = 3,
    .log2_chroma_w = 0,
    .log2_chroma_h = 0,
...
static enum AVPixelFormat get_pix_fmt_internal(const char *name)
{
    ...
    if (av_pix_fmt_descriptors_yuv420p.name &&
            (!strcmp(av_pix_fmt_descriptors_yuv420p.name, name) ||
             av_match_name(name, av_pix_fmt_descriptors_yuv420p.alias)))
            return AV_PIX_FMT_YUV420P;
    if (av_pix_fmt_descriptors_rgb565le.name &&
            (!strcmp(av_pix_fmt_descriptors_rgb565le.name, name) ||
             av_match_name(name, av_pix_fmt_descriptors_rgb565le.alias)))
            return AV_PIX_FMT_RGB565LE;
    if (av_pix_fmt_descriptors_rgb24.name &&
            (!strcmp(av_pix_fmt_descriptors_rgb24.name, name) ||
             av_match_name(name, av_pix_fmt_descriptors_rgb24.alias)))
            return AV_PIX_FMT_RGB24;
    if (av_pix_fmt_descriptors_bgr24.name &&
            (!strcmp(av_pix_fmt_descriptors_bgr24.name, name) ||
             av_match_name(name, av_pix_fmt_descriptors_bgr24.alias)))
            return AV_PIX_FMT_BGR24;
    rt_kprintf("get_pix_fmt_internal error=%s\n", name);
    RT_ASSERT(0);
...
{
    if ((unsigned)pix_fmt == AV_PIX_FMT_YUV420P)
        return av_pix_fmt_descriptors_yuv420p.name;
    if ((unsigned)pix_fmt == AV_PIX_FMT_RGB565LE)
        return av_pix_fmt_descriptors_rgb565le.name;
    if ((unsigned)pix_fmt == AV_PIX_FMT_RGB24)
        return av_pix_fmt_descriptors_rgb24.name;
    if ((unsigned)pix_fmt == AV_PIX_FMT_BGR24)
        return av_pix_fmt_descriptors_bgr24.name;
    rt_kprintf("av_get_pix_fmt_name error=%d\n", pix_fmt);
    RT_ASSERT(0);
    return NULL;
}
#else
...
const AVPixFmtDescriptor *av_pix_fmt_desc_get(enum AVPixelFormat pix_fmt)
{
    ...
    if (pix_fmt == AV_PIX_FMT_YUV420P)
        return &av_pix_fmt_descriptors_yuv420p;

    if (pix_fmt == AV_PIX_FMT_RGB565LE)
        return &av_pix_fmt_descriptors_rgb565le;

    if (pix_fmt == AV_PIX_FMT_RGB24)
        return &av_pix_fmt_descriptors_rgb24;

    if (pix_fmt == AV_PIX_FMT_BGR24)
        return &av_pix_fmt_descriptors_bgr24;

    rt_kprintf("av_pix_fmt_desc_get error=%d\n",pix_fmt);
    RT_ASSERT(0);
    return NULL;
}
#else
...
#if !MINI_PIXDESC_SIZE || CONFIG_H263_DECODER
