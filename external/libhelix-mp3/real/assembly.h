...
__STATIC_FORCEINLINE int CLZ(int x)
{
	int numZeros;
...
__STATIC_FORCEINLINE int FASTABS(int x) 
{
	int sign;
...
__STATIC_FORCEINLINE int MULSHIFT32(int x, int y) {
	return (((Word64)x * y)) >> 32;
}

__STATIC_FORCEINLINE Word64 SAR64(Word64 x, int n) {
	return x >>= n;
}

__STATIC_FORCEINLINE Word64 MADD64(Word64 sum64, int x, int y)
{
