...
SFT_Font *
sft_loadmem(SFT_Font *font, const void *mem, size_t size)
{
    if (size > UINT32_MAX)
    {
        return NULL;
...
static void *
_bsearch(const void *key, const void *base,
            ...
            SFT_Kerning *kerning)
{
    ...
    while (numTables > 0)
    {
        ...
        if (format == 0 && (flags & HORIZONTAL_KERNING) && !(flags & MINIMUM_KERNING))
        {
            ...
            if ((match = _bsearch(key, sft->font->memory + offset,
...
map_file(SFT_Font *font, const char *filename)
{
    ...
    font->memory = malloc(info.st_size);
    read(fd, (uint8_t *)font->memory, info.st_size);
...
#ifdef USING_MICROLIB
    #define nextafterf(a,b) ((float)a>b)?(float)a:b
#endif
...
clip_points(unsigned int numPts, Point *points, int width, int height)
{
    ...
    for (i = 0; i < numPts; ++i)
    {
        ...
        if (pt.x >= width)
        {
            points[i].x = nextafterf(width, 0.0f);
        }
        if (pt.y < 0.0f)
        {
            points[i].y = 0.0f;
        ...
        if (pt.y >= height)
        {
            points[i].y = nextafterf(height, 0.0f);
        }
    }
}
...
gettable(SFT_Font *font, char tag[4], uint_fast32_t *offset)
{
    ...
    if (!is_safe_offset(font, 12, (uint_fast32_t) numTables * 16))
        return -1;
    if (!(match = _bsearch(tag, font->memory + 12, numTables, 16, cmpu32)))
        return -1;
...
decode_contour(uint8_t *flags, uint_fast16_t basePoint, uint_fast16_t count, Outline *outl)
{
    ...
    RT_ASSERT(basePoint <= UINT16_MAX - count);
...
#define STACK_SIZE 10
    ...
    for (;;)
    {
        ...
        else
        {
            ...
            stack[top++] = (Curve)
            {
                curve.beg, pivot, ctrl0
            };
            curve = (Curve)
            {
                pivot, curve.end, ctrl1
            };
        }
    }
    return 0;
...
draw_line(Raster buf, Point origin, Point goal)
{
    ...
    crossingIncr.x = dir.x ? fabs(1.0f / delta.x) : 1.0f;
    crossingIncr.y = fabs(1.0f / delta.y);
...
#undef MEMORY_OVERFLOW_WORKAROUD
static int
...
#ifdef MEMORY_OVERFLOW_WORKAROUD
    STACK_ALLOC(cells, Cell, /*128 * 128*/32, numPixels + 10);
#else
    STACK_ALLOC(cells, Cell, /*128 * 128*/32, numPixels + 1);
#endif
