...
{
    audio_client_t client = *((audio_client_t *)reserved);
...
static void lc3_decoder_thread(void *arg1, void *arg2, void *arg3)
{
    ...
    pa.write_samplerate = LC3_MAX_SAMPLE_RATE;
    ...
    while (true)
    {
        ...
        for (size_t i = 0; i < ARRAY_SIZE(streams); i++)
        {
            ...
            for (uint8_t i = 0U; i < frames_blocks_per_sdu; i++)
            {
                ...
                uint32_t num_of_sample = LC3_NS(stream->lc3_decoder->dt,
                                                stream->lc3_decoder->sr_pcm);

                if (!client)
                {
                    pa.write_samplerate = (stream->lc3_decoder->sr_pcm - LC3_SRATE_8K + 1) * 8000;
                    pa.write_channnel_num = frames_per_block;
