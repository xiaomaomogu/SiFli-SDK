@使用指南
    ...
    @介绍：pair工程是基于RTthreadOS，作为BLE peripheral role进行配对的示例。该工程可运行在SF32LB55X/SF32LB58X/SF32LB56X/SF32LB52X等系列上。
                - 包含了创建自定义的GATT service，进行BLE的广播、连接，以及连接后基本的GATT数据交互。
                - 设置IO，作为peripheral role进行配对和要求central配对。
    ...
    @手机端建议：
		1. 推荐在Android端用nRF Connect进行BLE测试，可以方便发起配对。
