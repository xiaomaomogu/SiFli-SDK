...
RT_USED const struct sec_configuration sec_config =
{
    ...
    .ftab[4] = {.base = FLASH_USER_CODE_START_ADDR,  .size = 0x80000,               .xip_base = FLASH_USER_CODE_START_ADDR, .flags = 0},
    .ftab[5] = {.base = FLASH_BOOT_PATCH_START_ADDR, .size = FLASH_BOOT_PATCH_SIZE, .xip_base = BOOTLOADER_PATCH_CODE_ADDR, .flags = 0},
    .ftab[8] = {.base = FLASH_USER_CODE_START_ADDR,  .size = 0x80000,               .xip_base = FLASH_USER_CODE_START_ADDR, .flags = 0},
    .ftab[9] = {.base = BOOTLOADER_PATCH_CODE_ADDR,  .size = FLASH_BOOT_PATCH_SIZE, .xip_base = BOOTLOADER_PATCH_CODE_ADDR, .flags = 0},
    .imgs[DFU_FLASH_IMG_IDX(DFU_FLASH_IMG_HCPU)] = {.length = 0x80000, .blksize = 512, .flags = DFU_FLAG_AUTO},
    .imgs[DFU_FLASH_IMG_IDX(DFU_FLASH_IMG_LCPU)] = {.length = 0xFFFFFFFF},
    .imgs[DFU_FLASH_IMG_IDX(DFU_FLASH_IMG_BL)] = {.length = 0xFFFFFFFF},
    ...
    .running_imgs[CORE_BL] = (struct image_header_enc *)0xFFFFFFFF,
