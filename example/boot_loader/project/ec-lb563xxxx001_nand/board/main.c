...
static void config_psram_pinmux(uint8_t id)
{
    if (id == 3 || id == 4)
    {
...
void boot_images()
{
    ...
    //uint32_t sip1 = SFPIN_SIP1_WINB_HYP64;
    //uint32_t sip2 = BSP_Get_Sip2_Mode();

    if ((sip1 == SFPIN_SIP1_WINB_HYP64) || (sip1 == SFPIN_SIP1_WINB_HYP32)) // no sip infor, chip should not pass ATE
    {
        //HAL_PIN_SetFlash1_WithMode(3);
        ...
        qspi_cfg.SpiMode = SPI_MODE_HBPSRAM; //SPI_MODE_PSRAM;
    }
    else
    {
    ...
    config_psram_pinmux(sip1);


    qspi_cfg.Instance = (MPI_TypeDef *)hwp_qspi1;
    //qspi_cfg.SpiMode = SPI_MODE_HBPSRAM;
    qspi_cfg.line = 0;
    qspi_cfg.msize = 8;
    qspi_cfg.base = QSPI1_MEM_BASE;
    handle.wakeup = 0;
    HAL_MPI_PSRAM_Init(&handle, &qspi_cfg, 1);
