...
uint32_t boot_get_flash_start_addr(boot_flash_t flash)
{
...
static uint32_t boot_flash_init(boot_flash_t flash)
{
    // initial mpi5 for ota
    init_mpi5();

    if ((boot_get_flash_start_addr(flash) == BOOT_INVALID_ADDR) || rt_nand_init() < 0)
