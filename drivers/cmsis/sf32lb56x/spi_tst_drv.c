...
{
    hwp_spi3->TOP_CTRL &= ~SPI_TOP_CTRL_SSE_Msk;
}
...
void clear_spi3_sph()
{
    hwp_spi3->TOP_CTRL &= ~SPI_TOP_CTRL_SPH_Msk;
}

void set_spi3_spo()
{
    hwp_spi3->TOP_CTRL |= SPI_TOP_CTRL_SPO;

}

void clear_spi3_spo()
{
    hwp_spi3->TOP_CTRL &= ~SPI_TOP_CTRL_SPO_Msk;

}

void set_spi3_clkdiv(uint8_t div)
{
    hwp_spi3->CLK_CTRL &= (~SPI_CLK_CTRL_CLK_DIV_Msk);
    hwp_spi3->CLK_CTRL |= (div << SPI_CLK_CTRL_CLK_DIV_Pos);
}
