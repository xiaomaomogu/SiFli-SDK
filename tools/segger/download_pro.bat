if exist %1\ER_IROM1 (
    ...
    jlink.exe -device SF32LB58X -if SWD -speed 10000 -autoconnect 1 -CommandFile %1\download.jlink
)else (
    copy %1 %~dpn1.hex
    echo r > download.jlink
    echo loadfile %~dpn1.hex >> download.jlink
    echo exit >> download.jlink 
    jlink.exe -device SF32LB58X -if SWD -speed 10000 -autoconnect 1 -CommandFile download.jlink
)
