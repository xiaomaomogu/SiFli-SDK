...
set TEST_WORKSPACE=Thinker_COMMON
...
call Thinker.exe --exit 1 --dutport %UART_H_PORT% --rtdport %UART_L_PORT% --dutport2 %UART_L_PORT% --noreset --switchtype %RTYPE% --switchport %RELAY_PORT%  --dutrchan %RCHAN% --fixcase %WORKSPACE%\%CASE_PATH% --retexcel %RET_EXCEL% %RET_ROW% %RET_COL% --logpath %RESULT_DIR%\log
if %errorlevel% NEQ 0 (
    SET TEST_RESULT=1
    goto ERREND
)
