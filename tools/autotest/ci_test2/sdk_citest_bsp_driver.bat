...
set TEST_WORKSPACE=Thinker_BSP
...
if not "%BSP_PRJ_L%"=="" (
    ...
    set compile_path=%BSP_PRJ_L%
    cd !compile_path!
    echo ****** !compile_path! **********
    echo,-----------Build [%BSP_PRJ_L%] start------------
    call scons -j8
    if !errorlevel! neq 0 goto BUILD_FAIL
    echo,-----------Build [%BSP_PRJ_L%] end------------
)

if not "%BSP_PRJ_H%"=="" (
    echo ****** set compile_path **********
    set compile_path=%BSP_PRJ_H%
    cd !compile_path!
    echo ****** !compile_path! **********
    echo,-----------Build [%BSP_PRJ_H%] start------------
    call scons -j8
    if !errorlevel! neq 0 goto BUILD_FAIL
    echo,-----------Build [%BSP_PRJ_H%] end------------
) else (
    echo ****** not configure bsp driver prj **********
    SET BUILD_RESULT=1
    goto BUILD_END
)
...
if %BUILD_RESULT% equ 0 (
    robocopy  %compile_path%\build\ %RESULT_DIR% *.bin *.hex  *.ini  /S /LEV:3
) else (
