...
class Shell():
    def __init__(self, shell: str, deactivate_cmd: str, new_sifli_sdk_env: Dict[str,str]):
        self.shell = shell
    ...
    def expanded_env(self) -> Dict[str, str]:
        expanded_env = self.new_sifli_sdk_env.copy()

        if 'PATH' not in expanded_env:
            return expanded_env

        # The PATH returned by sifli_sdk_tools.py export is not expanded.
...
class UnixShell(Shell):
    def __init__(self, shell: str, deactivate_cmd: str, new_sifli_sdk_env: Dict[str,str]):
        super().__init__(shell, deactivate_cmd, new_sifli_sdk_env)
        ...
        self.new_sifli_sdk_env['SIFLI_SDK_TOOLS_INSTALL_CMD'] = os.path.join(conf.SIFLI_SDK_PATH, 'install.sh')
        self.new_sifli_sdk_env['SIFLI_SDK_TOOLS_EXPORT_CMD'] = os.path.join(conf.SIFLI_SDK_PATH, 'export.sh')
    ...
    def export_file(self, fd: TextIO) -> None:
        ...
        for var, value in self.new_sifli_sdk_env.items():
    ...
    def autocompletion(self) -> str:
        ...
        autocom = textwrap.dedent(f"""
            ...
            then
                ...
                if ! eval "$(env LANG=en _SDK.PY_COMPLETE={bash_source} sdk.py)"
                then
                    echo "$WARNING_MSG"
                fi
                export SIFLI_SDK_PY_COMP_WORDBREAKS="$COMP_WORDBREAKS"
            fi
            """)
...
class ZshShell(UnixShell):
    ...
    def autocompletion(self) -> str:
        ...
        autocom = textwrap.dedent(f"""
            ...
            if ! eval "$(env _SDK.PY_COMPLETE={zsh_source} sdk.py)"
            then
                echo "$WARNING_MSG"
            fi
            """)
...
class FishShell(UnixShell):
    def __init__(self, shell: str, deactivate_cmd: str, new_sifli_sdk_env: Dict[str,str]):
        super().__init__(shell, deactivate_cmd, new_sifli_sdk_env)
        self.new_sifli_sdk_env['SIFLI_SDK_TOOLS_INSTALL_CMD'] = os.path.join(conf.SIFLI_SDK_PATH, 'install.fish')
        self.new_sifli_sdk_env['SIFLI_SDK_TOOLS_EXPORT_CMD'] = os.path.join(conf.SIFLI_SDK_PATH, 'export.fish')

    @status_message('Shell completion', die_on_err=False)
    def autocompletion(self) -> str:
        env = self.expanded_env()
        env['LANG'] = 'en'
        env['_SDK.PY_COMPLETE'] = 'fish_source' if self.click_ver() >= 8 else 'source_fish'
        stdout: str = run_cmd([sys.executable, conf.SIFLI_SDK_PY], env=env)
        return stdout
...
class PowerShell(Shell):
    def __init__(self, shell: str, deactivate_cmd: str, new_sifli_sdk_env: Dict[str,str]):
        ...
        self.new_sifli_sdk_env['SIFLI_SDK_TOOLS_INSTALL_CMD'] = os.path.join(conf.SIFLI_SDK_PATH, 'install.ps1')
        self.new_sifli_sdk_env['SIFLI_SDK_TOOLS_EXPORT_CMD'] = os.path.join(conf.SIFLI_SDK_PATH, 'export.ps1')

    def get_functions(self) -> str:
        return '\n'.join([
            r'function sdk.py { &python "$Env:SIFLI_SDK_PATH\tools\sdk.py" $args }',
            ...
            r'function global:otatool.py { &python "$Env:SIFLI_SDK_PATH\components\app_update\otatool.py" $args }',
            r'function global:parttool.py { &python "$Env:SIFLI_SDK_PATH\components\partition_table\parttool.py" $args }',
        ])
...
class WinCmd(Shell):
    def __init__(self, shell: str, deactivate_cmd: str, new_sifli_sdk_env: Dict[str,str]):
        ...
        self.new_sifli_sdk_env['SIFLI_SDK_TOOLS_INSTALL_CMD'] = os.path.join(conf.SIFLI_SDK_PATH, 'install.bat')
        self.new_sifli_sdk_env['SIFLI_SDK_TOOLS_EXPORT_CMD'] = os.path.join(conf.SIFLI_SDK_PATH, 'export.bat')
        self.new_sifli_sdk_env['SIFLI_SDK_TOOLS_JSON_PATH'] = os.path.join(conf.ISIFLI_SDK_PATH, 'tools', 'tools.json')
        self.new_sifli_sdk_env['SIFLI_SDK_TOOLS_PY_PATH'] = conf.SIFLI_SDK_TOOLS_PY

    def get_functions(self) -> str:
        return '\n'.join([
            r'DOSKEY sdk.py=python.exe "%SIFLI_SDK_PATH%\tools\sdk.py" $*',
            ...
            r'DOSKEY otatool.py=python.exe "%SIFLI_SDK_PATH%\components\app_update\otatool.py" $*',
            r'DOSKEY parttool.py=python.exe "%SIFLI_SDK_PATH%\components\partition_table\parttool.py" $*',
        ])
