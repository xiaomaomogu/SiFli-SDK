...
class Kconfig(object):
    ...
    def _init(self, filename, warn, warn_to_stderr, encoding):
        ...
        self.warn_unsatisfied_deps = True

        self.warnings = []

        # symbol definitions needs to be excluded when saving as minimal conf
        self.excluding_sym = []
    ...
    def _autoconf_contents(self, header):
        ...
        if os.path.isfile('rtconfig_project.h'):
            add ('#include "rtconfig_project.h"\n') 
        #else:
        #    print(os.getcwd())
        add("#endif\n")
        return "".join(chunks)
    ...
    def _min_config_contents(self, header):
        ...
        for sym in self.unique_defined_syms:
            ...
            if sym.str_value == sym._str_default():
                skip = True
                for i in self.excluding_sym:
                    if sym.name in i:
                        # Though value is same as default, but it might be different from that in excluding_sym,
                        # so still need to include it in minimal config file
                        skip = False
                        break
        ...
        for i in self.excluding_sym:
            chunks = [chunk for chunk in chunks if i.strip() != chunk.strip()]
        return "".join(chunks)
...
class Symbol(object):
    ...
    def _warn_select_unsatisfied_deps(self):
        ...
        if not self.kconfig.warn_unsatisfied_deps:
            return
...
def standard_kconfig(kconfig):
    ...
    """
    return Kconfig(kconfig, suppress_traceback=True)
